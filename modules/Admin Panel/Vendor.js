...
const Vendor = () => {
    ...
    const approvalHandler = (data,status) =>{
        ...
        setLoading(true)
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL}admin/verifyVendor/${id}`, requestOptions)
        .then(response => response.json())
        .then(result =>{
            fetch(`${process.env.NEXT_PUBLIC_BASE_URL}admin/getAllVendors`,{
                method: 'GET', 
                headers: myHeaders,
            })
            .then(response => response.json())
    ...
    useEffect(()=>{
        ...
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL}admin/getAllVendors`, requestOptions)
        .then(response => response.json())
        .then(result =>{
            setData(result.data)
            setLoading(false)
    ...
    <>
        ...
        <div className='vendor-container'>
            <div className='d-flex d-align-center d-justify-space-between'>
                <h4 className='f-600 text-primary mt-24 mb-24'>Vendors</h4>
            </div>
            <div className={`${styles["dashboard-table-section-scroll"]}`}>
                <div className={`${styles["dashboard-table-wrapper"]}`}>
                    <div className={`${styles["dashboard-table-column"]}  bg-orange d-flex d-align-center`}>
                        ...
                        <span className='font-16 f-600 d-flex'>Email</span>
                    ...
                    {data && data.map((item)=>(
                        <div className={`${styles["dashboard-table-column"]} ${styles["dashboard-table-column-data"]} d-flex d-align-center`}>      
                            <span className='font-14 f-600 d-flex word-break'>{item.name}</span>
                            <span className='font-14 f-500 d-flex'>{item.email}</span>
        ...
        {add && 
            <Modal modalClass="modal-verify">
                <ApprovalModal submithandler={approvalHandler} handler={modalHandler}></ApprovalModal>
            </Modal>
        }
         {accept && 
            <Modal modalClass="modal-verify">
                <Accepted name="Vendor" submithandler={approvalHandler} handler={modalHandler2}></Accepted>
            </Modal>
        }
    </>
  )
