...
const SellNFT = () => {
  ...
  const sellNftWeb3 = async () => {
    ...
    if (typeof window.ethereum !== 'undefined') {
      ...
      try {
        await contract.placeNFTForSale(data.tokenId, price).then((response) => {
  ...
  return (
    <div className={`${styles['sell-nft-wrapper']}`}>
      ...
      <div className={`d-flex ${styles['sell-nft-img-content-wrapper']}`}>
        <div
          className={`d-flex d-justify-center bg-common rounded-16 ${styles['sell-nftimg-wrapper']}`}
        >
          <img
            className={`col-6 ${styles['sell-nft-UI-img']}`}
            src={data.imageUrl}
          ></img>
        </div>
        <div className={`col-6 ${styles['sell-nft-content']}`}>
          ...
          <h5 className={`f-600 l-27 ${styles['contract-address-heading']}`}>
            Contract Address
          </h5>
          <h5 className={`f-400 l-27 ${styles['contract-address']}`}>
            0x75d87f709B5E74F049271D9d82816231dCEE1eEd
          </h5>
      ...
      <div className={`${styles['set-price-wrapper']}`}>
        ...
        <form className="col-8" onSubmit={formSubmit}>
          ...
          <div className={`${styles['expire-date']}`}>
            <h4 className={`f-600 mb-32 ${styles['expiry-date-h4']}`}>
              Expiration Date
            </h4>
            <DropDown
            ...
            <div className={`d-flex d-flex-column ${styles['royalty-fee']}`}>
              <h3 className={`font-31 f-600 l-39 ${styles['royalty-fee-h3']}`}>
                Royalty Fee
              </h3>
              <h4 className={`font-24 f-400 l-33 ${styles['royalty-fee-h4']}`}>
                A royalty payment gives a percentage of of the sale price to the
                orginal creator/ beneficiary each time the NFT is sold on our
